...
{{ define "discord" }}
        {{ if gt (len .Alerts.Firing) 0 }}**Firing**{{ template "discord.default.message" .Alerts.Firing }}{{ end }}
        {{ if gt (len .Alerts.Resolved) 0 }}**Resolved**{{ template "discord.resolved.message" .Alerts.Resolved }}{{ end }}
{{ end }}